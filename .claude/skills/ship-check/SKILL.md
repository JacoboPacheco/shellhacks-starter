...
9. Devpost submission (you can't see it — ask the user to confirm each in one AskUserQuestion): the GitHub repo link is attached; at least one Discord tag is in the submission; every sponsor challenge from PITCH.md is opted into (and Best First-Time Hacker if eligible); the 3-minute demo video is uploaded or linked; the Devpost account uses the same email as the ShellHacks registration. Submissions close when hacking ends.
