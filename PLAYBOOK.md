...
2. Timeline still has placeholders: write K = the first `Now:` line of this session and E = the date in the E placeholder, both marked `(ASSUMED — correct me)`, say so in one line, and continue; the human corrects them if wrong. Never spend a question on it.
...
- The idea's core piece proves impossible — not merely hard — before K+4: say so and offer the `RUNNER-UP` line from CLAUDE.md → Decisions in one AskUserQuestion (Away mode: a `WAITING ON YOU` line). Accepted: delete SPEC.md and the idea's own routers, models, and screens (git history keeps them; the prompts are fine, the human is present), commit `Abandon <idea>`, replace Idea with the runner-up text, delete that line, write `PHASE: 1 — switching to runner-up`, and tell the human: `/model` to the bigger model, then `/spec <runner-up>`. After K+4 never switch ideas; cut scope instead.
