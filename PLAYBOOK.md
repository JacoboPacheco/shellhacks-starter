...
2. Timeline still has placeholders: write K = now and E = the date in the E placeholder, both marked `(ASSUMED — correct me)`, say so in one line, and continue; the human corrects them if wrong. Never spend a question on it.
...
- The idea's core piece proves impossible — not merely hard — before K+4: say so and offer the `RUNNER-UP` line from CLAUDE.md → Decisions in one AskUserQuestion (Away mode: a `WAITING ON YOU` line). Accepted: `git mv SPEC.md docs/SPEC-abandoned.md`, replace Idea with the runner-up text, delete that line, write `PHASE: 1 — switching to runner-up`, and tell the human to type `/spec <runner-up>`. After K+4 never switch ideas; cut scope instead.
